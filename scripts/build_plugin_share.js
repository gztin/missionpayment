...
const pluginVersion = "0.2.2+codex.20260605090300";
...
const pluginJson = {
  ...
  interface: {
    ...
    longDescription: "Mission Invoice 會在 Codex 任務消耗 token 後，透過本機 MCP 流程記錄一張估算的 Token 電子發票，內容包含模型、耗時、明細項目與發票連結。一般任務不需要加任何前綴；只要 Mission Invoice 為 ON，就會在任務完成前自動開立發票並在回覆中附上連結。專案第一次使用時可執行 /mission setup：外掛會先詢問是否將 Mission Invoice 規則加入該專案的 AGENTS.md，只有在使用者同意後才會寫入，且會使用可辨識區塊避免重複。只有切換開票功能時才需要使用指令：/mission on 會開啟自動開票，/mission off 會關閉自動開票。/mission model list 可查看支援的參考模型，/mission model <model> 可切換 credits 估算用的參考費率。/mission runtime 可查看目前 runtime 版本，/mission update 可更新發票核心邏輯而不改動 plugin manifest。外掛會產生本機靜態 HTML 發票、歷史帳單與統計資訊，不需要啟動本機 dashboard server。所有資料都只儲存在本機 ~/.codex-token-billing，其中 settings 為全域共用，usage-log 與 receipts 會依專案路徑分開保存。此插件不會讀取官方 Codex 帳號方案、實際用量或帳單資料。",
    developerName: "本機開發者",
    category: "生產力",
    capabilities: [
      "互動式",
